...
export default function HistoryPage(){
  ...
  return <AppShell><PageHeader title="Assessment History" description="Review locally saved voice, biomedical, and EEG screening results from this browser." /><Card><CardContent className="p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-3 size-5 text-slate-400"/><Input className="pl-10" placeholder="Search by assessment ID" value={search} onChange={e=>setSearch(e.target.value)} /></div><select className="h-11 rounded-md border border-slate-300 bg-white px-4 text-sm" value={type} onChange={e=>setType(e.target.value)}><option>All</option><option>Voice</option><option value="General">Biomedical</option><option>EEG</option></select></div></CardContent></Card>
    <Card className="mt-6 overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[800px] text-left text-sm"><thead className="bg-slate-100 text-xs uppercase text-slate-500"><tr>{["Date","Assessment ID","Type","Result","Score","Level","Actions"].map(label=><th key={label} className="px-5 py-4 font-semibold">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-200">{filtered.map(item=>{const cell=resultCell(item);return <tr key={item.id} className="hover:bg-slate-50"><td className="px-5 py-4">{new Date(item.createdAt).toLocaleDateString()}</td><td className="px-5 py-4 font-mono text-xs">{item.id}</td><td className="px-5 py-4">{typeLabel[item.type]}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${cell.tone}`}>{cell.label}</span></td><td className="px-5 py-4">{cell.score}</td><td className="px-5 py-4">{cell.confidence}</td><td className="px-5 py-4"><div className="flex items-center gap-1"><Button variant="ghost" size="sm" onClick={()=>view(item)}>View</Button><Button variant="ghost" size="icon" onClick={()=>download(item)} aria-label={`Download ${item.id}`}><Download className="size-4" /></Button></div></td></tr>;})}</tbody></table>{filtered.length===0&&<div className="p-12 text-center text-sm text-slate-500">No assessments match these filters. Complete an assessment to populate history.</div>}</div></Card>
  </AppShell>;
}
