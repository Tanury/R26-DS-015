...
async function analyzeDatPipeline(file) {
  const form = new FormData(); form.append("file", file);
  const res  = await fetch("http://localhost:8000/api/pd/analyze/dat/pipeline", { method: "POST", body: form });
  if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.detail || "DaTscan pipeline failed"); }
  return res.json();
}

async function analyzePdMRI(file) {
  const form = new FormData(); form.append("file", file);
  const res  = await fetch("http://localhost:8000/api/pd/analyze/mri", { method: "POST", body: form });
  if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.detail || "MRI analysis failed"); }
  return res.json();
}

async function analyzeMsMRI(file) {
  const form = new FormData(); form.append("file", file);
  const res  = await fetch("http://localhost:8000/api/ms/analyze/mri", { method: "POST", body: form });
  if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.detail || "MRI analysis failed"); }
  return res.json();
}

async function analyzeMsOct(volFile, matFile) {
  const form = new FormData(); form.append("vol_file", volFile); form.append("mat_file", matFile);
  const res  = await fetch("http://localhost:8000/api/ms/analyze/oct", { method: "POST", body: form });
  if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.detail || "OCT analysis failed"); }
  return res.json();
}
        ...
        <header style={{background:t.headerBg,padding:"0 28px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0 10px"}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <div>
                <div style={{fontSize:17,fontWeight:900,color:"#fff",fontFamily:"'DM Serif Display',serif",letterSpacing:-0.3}}>NeuroRisk AI</div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0 12px",borderTop:"1px solid rgba(255,255,255,0.15)"}}>
            ...
            <div style={{display:"flex",gap:5,marginLeft:4}}>
              {[...Array(6)].map((_,i)=>(
                <span key={i} style={{fontSize:15,display:"inline-block",animation:`float ${2+i*0.25}s ease-in-out infinite`,animationDelay:`${i*0.18}s`}}>{t.decorSymbol}</span>
              ))}
            </div>
          </div>
        </header>
